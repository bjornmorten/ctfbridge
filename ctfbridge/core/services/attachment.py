...
class CoreAttachmentService(AttachmentService):
    ...
    async def download(
        ...
        try:
            ...
            await self._save_stream_to_file(resp, save_path)
    ...
    async def _fetch(self, url: str) -> httpx.Response:
        if self._is_external_url(url):
            logger.debug("Using external HTTP client for %s", url)
            return await self._external_http.get(url)
        else:
            logger.debug("Using authenticated session for %s", url)
            return await self._client._http.get(url)
    ...
    async def _save_stream_to_file(self, response: httpx.Response, path: str):
        try:
            ...
            raise IOError(f"Failed to save file to {path}: {e}") from e
